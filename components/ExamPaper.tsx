  ...
  return (
    <>
        ...
        >
                ...
                >
                    ...
                    <div className="relative dropdown-container">
                        ...
                        <button onClick={() => handleFinishClick(q.id)} className="flex items-center gap-1 px-3 py-1.5 rounded-full transition-colors bg-green-100 text-green-700 hover:bg-green-200">
                            <PaperIcons.Check /> <span className="text-[10px] font-bold">إنهاء</span>
                        </button>
                        ) : (
                        <button onClick={() => setActiveMenuId(isMenuOpen ? null : q.id)} className="flex items-center gap-1 px-2 py-1.5 rounded-full transition-colors text-gray-700 hover:bg-gray-100">
                            <PaperIcons.Edit /> <span className="text-[10px] font-bold">تعديل</span> <PaperIcons.ChevronDown />
                        </button>
                        )}
                        ...
                        <div className={`shrink-0 order-2 flex flex-col items-center justify-center p-2 bg-gray-50 border border-gray-200 rounded-xl relative group/visual ${(!q.imageUrl && !q.svgCode) ? 'hidden' : 'flex'}`} style={{ width: q.imageWidth ? `${q.imageWidth}px` : '200px', height: q.imageHeight ? `${q.imageHeight}px` : 'auto' }}>
                            ...
                            <div className={`w-full bg-white rounded-lg p-2 shadow-sm border border-gray-100 relative group/svg ${q.imageHeight ? 'h-full' : 'mb-2'}`}>
                                ...
                                {isEditing && (
                                    ...
                                    <button onClick={() => setSvgEditModal({id: q.id, instruction: ''})} className="bg-purple-500 text-white w-6 h-6 rounded shadow hover:bg-purple-600 transition flex items-center justify-center text-xs" title="تعديل الشكل بالأوامر">✏️</button>
                            ...
                            <div className="relative w-full h-full group/image select-none">
                                ...
                                <>
                                    ...
                                    <button onClick={() => onUpdateQuestion(q.id, { ...q, imageUrl: undefined })} className="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded-full shadow hover:bg-red-600 transition flex items-center justify-center z-10" title="حذف الصورة"><PaperIcons.X /></button>
                                </>
                                )}
                            </div>
                            )}
                            {isEditing && (q.imageUrl || q.svgCode) && (
                                <div onMouseDown={(e) => startResize(e, 'question', q.id)} className="absolute bottom-[-8px] right-[-8px] w-6 h-6 bg-blue-500 rounded-full cursor-nwse-resize shadow-lg flex items-center justify-center z-30 hover:scale-110 transition-transform" title="اسحب لتغيير الطول والعرض">
            ...
            <div className="answer-key-section mt-12 border-2 border-red-400 bg-red-50/50 p-6 rounded-xl break-inside-avoid relative z-10">
                ...
                {needsAnswerUpdate && (
                    ...
                    <div>
                        <p className="font-bold">تم تعديل الأسئلة</p>
                        <p className="text-sm">يرجى الضغط على زر "تحديث التفسيرات" بالأسفل للتأكد من مطابقة الإجابات للأسئلة الجديدة قبل التصدير.</p>
                    </div>
                    </div>
                )}
                    ...
                    <div className="space-y-4">
                        ...
                        <div key={q.id} className="text-xs text-gray-700 border-b border-red-100 pb-3 bg-white/50 rounded p-2">
                            <div className="flex items-start gap-2">
                                <span className="font-bold text-red-600 shrink-0">تفسير س{i + 1}:</span>
                                <div className="flex-grow">
                                    {editingId === q.id ? (
                                        <RichTextEditor value={q.explanation} onChange={(val) => onUpdateQuestion(q.id, { ...q, explanation: val })} className="text-[12px]" rows={2} placeholder="اكتب التفسير..." />
                                    ) : (
                                        <MathRenderer content={q.explanation || "لا يوجد تفسير متاح."} inline />
                                    )}
                                </div>
                            </div>
                        </div>
      ...
      <div className="mt-8 no-print w-full max-w-[210mm] mx-auto flex flex-col md:flex-row gap-3 items-center justify-center border-2 border-dashed border-primary/40 rounded-xl p-4 hover:bg-blue-50 transition-all group">
          ...
          {onAddText && (
            <button onClick={onAddText} className="flex-1 text-gray-600 font-bold hover:text-gray-900 transition flex items-center justify-center gap-2 w-full md:w-auto h-10 border-r border-gray-300"><span className="text-xl">📝</span> إضافة نص</button>
          )}
